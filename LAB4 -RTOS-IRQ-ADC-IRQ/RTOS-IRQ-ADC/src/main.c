...
static void AFEC_pot_callback(void) {
  ...
  xQueueSendFromISR(xQueueTask, &adc, &xHigherPriorityTaskWoken);
}
...
static void task_proc(void *pvParameters) {
  ...
  adcData adc;
	...
	int i =0;
  while (1) {
    if (xQueueReceive(xQueueTask, &(adc), 1000)) {
		if (i>9){
			i =0;
		}
		v[i]=  adc.value;
...
int main(void) {
  ...
  xQueueTask = xQueueCreate(100, sizeof(adcData));
  if (xQueueADC == NULL)
